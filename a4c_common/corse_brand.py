...
def registry_entry_for_date(entry: Mapping | None, on_date=None) -> Mapping | None:
    """Select the verified identity period valid on ``on_date``.

    Old registries without temporal fields remain backward compatible. Once ``brand_valid_from``
    exists, earlier dates are resolved from ``brand_history`` rather than silently inheriting a
    newly observed brand.
    """
