...
* Shield exceptions apply only to TotalEnergies, only when the shield status
  has already been established from normally-fresh observations, and only when
  the last price is at the applicable cap.
* Corsica liveness: use the other principal fuel (Gazole <-> SP95).
* Mainland liveness: any recently declared fuel can prove that the station is
  still active.
* Double cap: if Gazole and SP95 are both at their effective caps, Rotterdam
  may suspend the 45-day expiry for both fuels while the Gazole cap remains
...
* A shield must never resurrect a value that was already stale when the
  relevant cap phase began. ``eligible_at_cap_entry`` enforces that guard.
"""
...
def recent_liveness(*, region_kind: str, target_fuel: str, activity_by_fuel: Mapping[str, datetime], day: date) -> bool:
    """Whether another declaration proves recent station activity."""
...
def evaluate(*, day: date, region_kind: str, target_fuel: str,
    ...
    if recent_liveness(region_kind=region_kind, target_fuel=target_fuel,
                       activity_by_fuel=activity_by_fuel, day=day):
