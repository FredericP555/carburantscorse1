...
class MainWriterWorkflowContracts(unittest.TestCase):
    def test_active_c1_main_writers_share_one_concurrency_group(self):
        for name in ("update-weekly.yml", "update-station-brands.yml"):
            text = (WORKFLOWS / name).read_text(encoding="utf-8")
            self.assertIn("group: c1-main-writers", text, name)
            self.assertIn("cancel-in-progress: false", text, name)

    def test_active_c1_main_writers_guard_remote_main_before_push(self):
        for name in ("update-weekly.yml", "update-station-brands.yml"):
            text = (WORKFLOWS / name).read_text(encoding="utf-8")
